...
async def get_products_by_category(category_id: str):
    # Cache key specific to the category
    cache_key = f"best_sellers:cat:{category_id}"
    ...
    async with AsyncSessionLocal() as session:
        # Re-using your CatBestSeller model logic
        stmt = (
            select(Product.product_id, Product.category_id)
            .join(CatBestSeller, Product.product_id == CatBestSeller.product_id)
            .where(CatBestSeller.category_id == category_id)
            .order_of(desc(CatBestSeller.unique_buyers))
            .limit(20)
        )
        result = await session.execute(stmt)
        products = [{"id": row.product_id, "category": row.category_id} for row in result.all()]
        ...
        if products:
            # Cache for 30 minutes as category trends are stable
            await redis_client.setex(cache_key, 1800, json.dumps(products)) 
            
        return products

async def get_all_categories():
    cached = await redis_client.get("catalog:categories")
    if cached:
        return json.loads(cached)

    async with AsyncSessionLocal() as session:
        # Get unique category IDs from the Product table
        stmt = select(Product.category_id).distinct()
        result = await session.execute(stmt)
        categories = [row.category_id for row in result.all()]
        
        await redis_client.setex("catalog:categories", 3600, json.dumps(categories))
        return categories

async def get_all_products():
    cached = await redis_client.get("catalog:all_products")
    if cached:
        return json.loads(cached)
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Cache for 1 hour
        await redis_client.setex("catalog:all_products", 3600, json.dumps(products))
        return products
...
async def clear_user_history(user_id: str):
    async with AsyncSessionLocal() as session:
        stmt = (
            update(BrowseHistory)
            .where(BrowseHistory.userid == user_id)
            .values(is_active=0)
        )
        await session.execute(stmt)
        await session.commit()
